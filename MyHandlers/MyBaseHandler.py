...
from MyRedisSession import RedisSession

class MyBaseHandler(tornado.web.RequestHandler):
    def __init__(self, *argc, **argkw):
        super(MyBaseHandler, self).__init__(*argc, **argkw)
        # 定义 handler 的 session, 注意，根据 HTTP 特点，每次访问都会初始化一个 Session 实例哦，这对于你后面的理解很重要
        self.session = RedisSession.Session(self.application.session_manager, self)
